...
func Main(cc echo.Context) error {
	...
	//Step 3. find privilege
	if privileges, count, err := t_rbac_privilege.GetPrivilegeList(c.Mysql(), offset, input.PageCount); err != nil {
		return c.RESULT_ERROR(ERR_PARAMETER_INVALID, err.Error())
	} else {
		output.Pages = 1
		output.Count = count
		log.PrintPreety("privileges=", privileges)
	...
	output.Code = 0
	output.Msg = "success"
	log.Debugf("get privilege list success")
